...
def send_telegram(message: str) -> bool:
    """Envia mensagem Telegram. Divide automaticamente se >4000 chars."""
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        print(message)
        return True

    # Split seguro em blocos de 4000 chars (margem de 96 para segurança)
    chunks = []
    ...
    for chunk in chunks:
        try:
            ...
            r.raise_for_status()
            if len(chunks) > 1:
                time.sleep(1)  # evita flood Telegram
...
def calculate_flip_target(
    ...
    beta_target = price * (1 + (beta or 1.0) * 0.12)
    anchors.append(("Beta recovery", beta_target))
    ...
    # ── Flag catalisador (Tavily > earnings > analyst > nenhum) ──────────────
...
def build_alert(
    ...
    if dip_score >= 8:
        score_badge = f"🔥 Score: {dip_score:.0f}/10"
    elif dip_score >= 6:
        score_badge = f"⭐ Score: {dip_score:.0f}/10"
    else:
        score_badge = f"📊 Score: {dip_score:.0f}/10"
...
def send_close_summary() -> None:
    ...
    WATCHDOG   = 25 * 60  # 25 minutos máximo para todo o resumo
    ...
    def _timed_out() -> bool:
        return (time.time() - start_time) > WATCHDOG
    ...
    for s in tier3_candidates:
        if _timed_out():
            logging.warning("Watchdog: Tier 3 truncado")
            break
        sym  = s["symbol"]
